...
class OssFileControllerTest {
    ...
    private void loginWorker(long workerId) {
    ...
    void sts_unauthenticated_returns401() throws Exception {
        mockMvc.perform(post("/api/worker/files/sts").param("biz", "avatar"))
    ...
    void signedUrl_unauthenticated_returns401() throws Exception {
        mockMvc.perform(get("/api/worker/files/signed-url").param("key", "worker/1/a.jpg"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(401));
    }
    ...
    void sts_illegalArgument_returns400() throws Exception {
        loginWorker(1L);
        when(ossStsService.issueSts(eq(1L), eq("bogus")))
                .thenThrow(new IllegalArgumentException("unsupported biz"));

        mockMvc.perform(post("/api/worker/files/sts").param("biz", "bogus"))
                .andExpect(status().isOk())
    ...
    void sts_illegalState_returns503() throws Exception {
        loginWorker(1L);
        when(ossStsService.issueSts(eq(1L), eq("avatar")))
    ...
    void sts_success_returns200AndData() throws Exception {
        ...
        vo.setPrefix("worker/1/");
        when(ossStsService.issueSts(eq(1L), eq("avatar"))).thenReturn(vo);

        mockMvc.perform(post("/api/worker/files/sts").param("biz", "avatar"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
                .andExpect(jsonPath("$.data.bucket").value("test-bucket"))
                .andExpect(jsonPath("$.data.prefix").value("worker/1/"));
    }

    @Test
    void signedUrl_illegalArgument_returns404() throws Exception {
        loginWorker(1L);
        when(ossSignedUrlService.getSignedUrl(eq(1L), eq("bad-key")))
                .thenThrow(new IllegalArgumentException("key not found"));

        mockMvc.perform(get("/api/worker/files/signed-url").param("key", "bad-key"))
                .andExpect(status().isOk())
    ...
    void signedUrl_security_returns403() throws Exception {
        loginWorker(1L);
        when(ossSignedUrlService.getSignedUrl(eq(1L), eq("worker/2/a.jpg")))
                .thenThrow(new SecurityException("cross worker access denied"));

        mockMvc.perform(get("/api/worker/files/signed-url").param("key", "worker/2/a.jpg"))
                .andExpect(status().isOk())
    ...
    void signedUrl_illegalState_returns503() throws Exception {
        loginWorker(1L);
        when(ossSignedUrlService.getSignedUrl(eq(1L), eq("worker/1/a.jpg")))
                .thenThrow(new IllegalStateException("oss not configured"));
    ...
    void signedUrl_success_returns200AndData() throws Exception {
        ...
        when(ossSignedUrlService.getSignedUrl(eq(1L), eq("worker/1/a.jpg"))).thenReturn(vo);
