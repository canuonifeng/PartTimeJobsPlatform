...
class JobApplicationMapperXmlTest {
    ...
    @Test
    void balanceTransactionMapperXml_shouldUseSingleTableQuery() throws IOException {
        String xml = new String(
                getClass().getClassLoader().getResourceAsStream("mapper/BalanceTransactionMapper.xml").readAllBytes(),
                StandardCharsets.UTF_8
        );

        assertThat(xml).contains("SELECT bt.*");
        assertThat(xml).contains("FROM balance_transactions bt");
        assertThat(xml).doesNotContain("LEFT JOIN");
    }
