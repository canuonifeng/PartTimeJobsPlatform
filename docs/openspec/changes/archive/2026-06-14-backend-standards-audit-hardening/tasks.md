...
- [x] 确认当前工作区企业端未提交改动是否继续保留，并以最终 diff 作为企业端扫描基线。
- [x] 复跑 Controller 硬规则扫描，按服务输出最终违规清单。
- [x] 复跑分页、Mapper 注解、敏感日志、Entity 关联字段扫描。
- [x] 按 must-fix、needs-db-confirm、legacy-debt 三类标记问题。
...
- [x] 复核当前企业端已改造 Controller，确保无 PUT/DELETE、Map 入参/返回、POST query/body 混用。
- [x] 决策 `FileController.upload` 是否作为 Multipart 例外，或改为 JSON 元数据流程。
- [x] 将 `WorkerProfileController.evaluateWorker` 的 `workerId` 合并进 `EvaluationCmd`。
- [x] 迁移 `EnterpriseMapper` 注解 SQL 到 XML。
- [x] 迁移或标记 `WorkerSyncMapper` 注解 SQL，确保列表场景只用批量查询。
- [x] 保持已修复的报名列表数据库分页和待办聚合查询，并复扫确认。
...
- [x] 核对实名认证、提现、企业 Entity 中疑似关联字段是否为真实表字段；非真实列迁移到 VO。

## 5. Frontend API Synchronization

- [x] 同步 `worker-uniapp` 受影响 API 方法的 HTTP method 和请求体。
- [x] 同步 `enterprise-uniapp`、`enterprise-pc` 受影响 API 方法的 HTTP method 和请求体。
- [x] 同步 `platform-pc` 受影响 API 方法的 HTTP method 和请求体。

## 6. Verification

- [x] 运行 `git diff --check`。
- [x] 运行 `cd c-service && JAVA_HOME=$(/usr/libexec/java_home -v 17 2>/dev/null) mvn test`。
- [x] 运行 `cd enterprise-service && JAVA_HOME=$(/usr/libexec/java_home -v 17 2>/dev/null) mvn test`。
- [x] 运行 `cd platform-service && JAVA_HOME=$(/usr/libexec/java_home -v 17 2>/dev/null) mvn test`。
- [x] 受影响前端分别运行对应 build。
- [x] 复跑静态扫描，并在交付说明中列出剩余 legacy-debt。
